...
if df_pro_ref.empty:
   ...
   # Préparation des données pour comparaison directe
    # CORRECTION : On ajoute 'entriesZ6Abs' ici pour qu'elle soit disponible ensuite
    df_compare = pd.concat([
        df_joueur[['distanceZ6Abs', 'distanceHSR', 'entriesZ6Abs']].assign(Groupe=f"Joueur ({joueur_cible})"),
        df_pro_ref[['distanceZ6Abs', 'distanceHSR', 'entriesZ6Abs']].assign(Groupe="Standard Pro")
    ])
    ...
    with col4:
        ...
        # --- PARTIE C : RÉPÉTABILITÉ (ENTRIES Z6) ---
    st.subheader("C. Répétabilité (Nombre d'efforts Z6)")
    
    col5, col6 = st.columns([2, 1])
    
    with col5:
        # On réutilise la fonction avec la colonne 'entriesZ6Abs'
        fig_entries = create_comparison_chart(df_compare, 'entriesZ6Abs', "Nombre de Sprints (Z6)", group_col='Groupe', unit="Répétitions")
        st.plotly_chart(fig_entries, use_container_width=True)
        
    with col6:
        st.caption("Analyse Répétabilité")
        # Calculs
        ent_j = df_joueur['entriesZ6Abs'].mean()
        ent_p = df_pro_ref['entriesZ6Abs'].mean()
        delta_ent = ent_j - ent_p
        
        # Affichage
        st.metric("Moyenne Joueur", f"{ent_j:.1f}")
        st.metric("Standard Pro", f"{ent_p:.1f}", delta_color="normal" if delta_ent >= 0 else "inverse", delta=f"{delta_ent:.1f}")
